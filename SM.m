function Decoding = SM(Decoding, Multi_Path, Nt, Nr, N_Relay, Data_Size, FFT_Size, Code_Rate, Modulation_Order, D, GI_Size, L, c, M, N, Z, LDPC_Iteration, SNR)
...
for Rx = 1 : Nr
    ...
    for Tx = 1 : Nt
        ...
        Receive(Tx,:) = conv(CP_Add(Tx,:) / sqrt(Nt) , h(Nt*(Rx-1)+Tx,:));
...
for t = 1 : U+1
    ...
    H_Reshape = reshape(H_, Nt, Nr*FFT_Size);
    
    for k = 1 : F
        
        H_k = H_Reshape(: , Nr*(k-1)+1 : Nr*k).';
